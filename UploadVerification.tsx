...
function UploadVerification() {
  ...
  const handleUploadDocument = async () => {
    ...
    try {
      ...
      const response = await fetch(`${API_URL}/auth/upload-verification-document`, {
        method: 'PUT',
        headers: {
  ...
  return (
    <SafeAreaView style={backgroundStyle}>
      ...
      <View style={{ alignItems: 'center', marginTop: 100, height: 600 }}>
        <Text style={{ color: 'black', marginBottom: 20 }}>Welcome, {name}</Text>

        <Text style={{ color: 'black', fontSize: 18, marginBottom: 20 }}>
          Please Upload Your Verification Document - Adhaar / Pan Card / Driving License 
        </Text>
